...
(function (window) {
    ...
    class Promise{
         ...
         * 
         */
        constructor(excutor){//接收一个执行器函数为参数
            const that = this;
         ...
         * @return 新的Promise
         */
        then(onResolved,onRejected){//then方法:接收成功和失败的回调
         ...
         * Promise 对象方法
         * @param 执行结果
         * @return 指定执行结果的Promise
         */
        static resolve(value){
            return new Promise((resolve,reject)=>{
                if(value instanceof Promise){//如果参数为Promise对象，获取此Promise对象的执行结果，决定最后的结果
         ...
         * Promise 对象方法
         * @param  失败的原因
         * @return 指定失败原因的Promise
         */
        static reject(reason){
            return new Promise((resolve,reject)=>{//直接执行失败
                reject(reason)
            })
        }
        /**
         ...
         */
        static race(promisesArray){
            return new Promise((resolve,reject)=>{
                promisesArray.forEach((pro,index)=>{
                    Promise.resolve(pro).then(value=>{//包装成Promise对象再点then
                        resolve(value)
                    },reason=>{
                        reject(reason)
                    })
